...
struct EngineTemperatureDerivativeTag end
const EngineTemperatureDual=ForwardDiff.Dual{EngineTemperatureDerivativeTag,Float64,1}
engine_temperature_dual(x)=EngineTemperatureDual(x,ForwardDiff.Partials((1.0,)))
...
function engine_ad_jacobian(problem;chunk=8,rate_evaluator=wdot!,clip_trials=true)
    return engine_ad_jacobian(problem,Val(chunk),rate_evaluator,clip_trials)
end
function engine_ad_jacobian(problem,::Val{N},rate_evaluator,clip_trials) where N
    ...
    return function (J,u,p,t)
        ...
        for k in 1:ns
            concentrations[k]==0 && (concentrations[k]=1e-100)
        end
